...
the posterior rescaled by the prior. The weight of the paper then falls on two results this
reading unlocks. The odds form places the weight of evidence, the Bayes factor, and the
information value inside one geometry (the weight of evidence at a point is the log of the
ratio of the bad and good CAP slopes), and the accuracy ratio, Somers' $D_{yx}$, and the
Gini $(2A-1)/(1-\pD)$ are revealed as one number computed three ways. Run in \emph{comparison
...
model-implied Ginis as a calibration diagnostic. A worked five-band example carries every
identity, and a kernel-density example confirms the construction in the continuous limit.
\end{abstract}
...
ratio, Somers' $D_{yx}$, and the Gini are the same number computed three ways
...
the portfolio-average risk a borrower at grade $x$ carries. Geometrically the standardized PD is
...
der Burgt (2019, Fig.~3) uses to turn the CAP into a calibration tool.
...
non-cancelling magnitude we use the integrated absolute distance between the two CAP curves,
...
integrated calibration error (ECE). The calibration reference in CAP
...
quantities off the densities, using none of the truth.
...
in the score whenever the truth is logistic; on this draw, fitting a line to the KDE-WOE over
...
class densities and attenuates the log-density-difference slope toward zero (the same
mechanism as measurement-error attenuation in regression), a bias that shrinks with the
...
The discrete five-band machinery and the continuous density machinery are the same identity at
two resolutions.
...
accuracy ratio, Somers' $D_{yx}$, and the Gini are one number computed three ways, and
